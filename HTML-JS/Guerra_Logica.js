...
let logDiv = document.getElementById("log-guerra");
let logHTML = `<details class="turno-log">
  <summary><strong>Turno ${turno} — Ataca ${atacante.nombre}</strong></summary>`;
...
/* -------------------------------elejimos aleatoriamente el objetivo del array unidades vivas ----------------------------------------- */
  ...
  // Agrupamos eventos por tipo atacante
...
logHTML += `</details>`;
// Agrupamos los logs en bloques de 10 turnos
...
if (!grupoDiv) {
  ...
  `;
  logDiv.innerHTML += grupoHTML;
  grupoDiv = document.getElementById(grupoId);
}

grupoDiv.innerHTML += logHTML;
